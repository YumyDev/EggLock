#include "CEggLockMenu.hpp"

#include <string>

#include <EggLockClient/CEggLockGUI.hpp>
#include <EggLockClient/Settings/Settings.hpp>
#include <EggLockClient/Settings/CSettingsJson.hpp>
#include <EggLockClient/Helpers/CPlayUISound.hpp>
#include <EggLockClient/Fonts/FontAwesomeIcon.hpp>

static CEggLockMenu g_CEggLockMenu{};

static auto ÑolorsGroupItemListGetter = []( void* vec , int idx , const char** out_text )
{
	auto pColorList = static_cast<CEggLockMenu::ColorItemsList_t*>( vec );
...
{
	auto& pSelected = GetEggLockMenu()->GetColorGroupNameFromSelectIndex( idx );
	*out_text = pSelected.c_str();
	return true;
};

auto CEggLockMenu::InitColors() -> void
{
...
auto CEggLockMenu::OnRenderMenu() -> void
{
...
auto CEggLockMenu::RenderLeftChild() -> void
{
	ImGui::BeginChild( XorStr( "EggLockChildLeft" ) , ImVec2( g_ChildSizeX , -1.f ) , true );
...
auto CEggLockMenu::RenderRightChild() -> void
{
	ImGui::BeginChild( XorStr( "EggLockChildRight" ) , ImVec2( g_ChildSizeX , ImGui::GetContentRegionAvail().y - 2.f ) , true );

	if ( ButtonIcon( ICON_FA_EYE , XorStr( "Visual##EggLockVisualIcon" ) , ImVec2( -1.f , g_ButtonSizeY ) ) )
	{
		m_bVisualWindow = !m_bVisualWindow;
	}

	if ( ButtonIcon( ICON_FA_PUZZLE_PIECE , XorStr( "Misc##EggLockMiscIcon" ) , ImVec2( -1.f , g_ButtonSizeY ) ) )
	{
		m_bMiscWindow = !m_bMiscWindow;
	}

	if ( ButtonIcon( ICON_FA_RAINBOW , XorStr( "Colors##EggLockColorsIcon" ) , ImVec2( -1.f , g_ButtonSizeY ) ) )
	{
...
auto CEggLockMenu::OnRenderVisual() -> void
{
	if ( m_bVisualWindow )
	{
		...
		{
			ImGui::BeginGroup();
			ImGui::BeginChild( XorStr( "##EggLockVisualLeftChild" ) , ImVec2( ImGui::GetContentRegionAvail().x , ImGui::GetContentRegionAvail().y ) , ImGuiChildFlags_Borders );
...
auto CEggLockMenu::OnRenderMisc() -> void
{
	if ( m_bMiscWindow )
	{
		...
		{
			ImGui::BeginGroup();
			ImGui::BeginChild( XorStr( "##EggLockMiscRightChild" ) , ImVec2( -1.f , ImGui::GetContentRegionAvail().y ) , ImGuiChildFlags_Borders );
...
auto CEggLockMenu::OnRenderColors() -> void
{
	if ( m_bColorsWindow )
	{
...
auto CEggLockMenu::ButtonIcon( const char8_t* szIcon , const char* szText , ImVec2 Size ) -> bool
{
	ImGui::PushFont( GetEggLockGUI()->m_pFontAwesomeIcons );
...
auto CEggLockMenu::RenderInputText( const char* szTitle , const char* szStrID , char* szBuffer , int BufferSize , float left_padding , float max_width ) -> bool
{
...
auto CEggLockMenu::RenderCheckBox( const char* szTitle , const char* szStrID , bool& SettingsItem ) -> bool
{
	if ( szTitle )
	{
		ImGui::AlignTextToFramePadding();
...
auto CEggLockMenu::RenderComboBox( const char* szTitle , const char* szStrID , int& v , const char* Items[] , int ItemsCount , float left_padding ) -> bool
{
	if ( szTitle )
	{
		ImGui::AlignTextToFramePadding();
...
auto CEggLockMenu::RenderSliderInt( const char* szTitle , const char* szStrID , int& v , int min , int max , float left_padding ) -> void
{
	if ( szTitle )
	{
		ImGui::AlignTextToFramePadding();
...
auto CEggLockMenu::RenderSliderFloat( const char* szTitle , const char* szStrID , float& v , float min , float max , float left_padding ) -> void
{
	if ( szTitle )
	{
		ImGui::AlignTextToFramePadding();
...
auto CEggLockMenu::RenderRandomBorderColor() -> void
{
...
auto CEggLockMenu::GetColorGroupNameFromSelectIndex( uint32_t SelectedGroup ) -> const std::string&
{
...
auto CEggLockMenu::GetColorListFromSelectIndex( uint32_t SelectedGroup ) -> ColorItemsList_t*
{
	if ( SelectedGroup >= 0 && SelectedGroup < m_ColorsManagerGroupList.size() )
	{
		const auto& pGroup = m_ColorsManagerGroupList[SelectedGroup];
...
auto CEggLockMenu::RenderListBoxColors( const char* Title , const char* szStrID , uint32_t* current_item , CEggLockMenu::ColorItemsList_t* pList , int height_in_items , float left_padding ) -> bool
{
	if ( Title )
	{
		ImGui::Text( Title );
...
auto CEggLockMenu::RenderComboBoxColorGroupList( const char* Title , const char* szStrID , uint32_t* current_item , int items_count , int popup_max_height_in_items , float left_padding ) -> bool
{
	if ( Title )
	{
		ImGui::Text( Title );
...
auto CEggLockMenu::PlayClick() -> void
{
	if ( Settings::Misc::MenuSounds )
		GetPlayUISound()->PlayUISound( UISound::UI_Click );
}

auto CEggLockMenu::PlayHover() -> void
{
	if ( Settings::Misc::MenuSounds )
		GetPlayUISound()->PlayUISound( UISound::UI_Hover );
}

auto GetEggLockMenu() -> CEggLockMenu*
{
	return &g_CEggLockMenu;
}
