...
public:
	auto OnInit() -> bool;
	auto LoadFromJson( const char* szJsonPath ) -> int;
	auto TryUpdateFromRemote() -> bool;
	auto ScanPatterns() -> bool;
...
public:
	...
	CBasePattern CCitadelInput_SetViewAngles = { VmpStr( "CCitadelInput::SetViewAngles" ) , VmpStr( "85 D2 75 ? 48 63 81 ? 0B 00 00 F2 41 0F 10 00" ) , CLIENT_DLL , 0 , SEARCH_TYPE_NONE };
	CBasePattern CGameEntitySystem_GetBaseEntity = { VmpStr( "CGameEntitySystem::GetBaseEntity" ) , VmpStr( "4C 8D 49 ? 81 FA ? ? ? ? 77" ) , CLIENT_DLL };
	CBasePattern CGameEntitySystem_GetHighestEntityIndex = { VmpStr( "CGameEntitySystem::GetHighestEntityIndex" ) , VmpStr( "8B 81 C4 00 00 00 89 02 C3" ) , CLIENT_DLL };
