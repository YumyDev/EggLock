...
static CEggLockGUI g_CEggLockGUI{};

IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler( HWND hwnd , UINT msg , WPARAM wParam , LPARAM lParam );

auto CEggLockGUI::OnInit( IDXGISwapChain* pSwapChain ) -> void
{
	DXGI_SWAP_CHAIN_DESC SwapChainDesc;

	if ( FAILED( pSwapChain->GetDevice( IID_PPV_ARGS( &m_pDevice ) ) ) )
	{
		DEV_LOG( "[error] CEggLockGUI::OnInit: #1\n" );
		return;
	}
	...
	{
		DEV_LOG( "[error] CEggLockGUI::OnInit: #2\n" );
		return;
	}
...
auto CEggLockGUI::OnDestroy() -> void
{
	SetWindowLongPtrA( m_hDeadLockWindow , GWLP_WNDPROC , (LONG_PTR)GetEggLockGUI()->m_WndProc_o );
...
auto CEggLockGUI::InitFont() -> void
{
...
auto CEggLockGUI::InitIndigoStyle() -> void
{
...
auto CEggLockGUI::InitVermillionStyle() -> void
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
...
auto CEggLockGUI::InitClassicSteamStyle() -> void
{
...
auto CEggLockGUI::UpdateStyle() -> void
{
	ImGui::SetCurrentContext( m_pImGuiContext );

	if ( Settings::Misc::MenuStyle == EEggLockGuiStyle::INDIGO )
		InitIndigoStyle();
	else if ( Settings::Misc::MenuStyle == EEggLockGuiStyle::VERMILLION )
		InitVermillionStyle();
	else if ( Settings::Misc::MenuStyle == EEggLockGuiStyle::CLASSIC_STEAM )
		InitClassicSteamStyle();
}

void CEggLockGUI::OnPresent( IDXGISwapChain* pSwapChain )
{
	if ( !m_bInit )
		OnInit( pSwapChain );
	else
...
void CEggLockGUI::OnResizeBuffers( IDXGISwapChain* pSwapChain )
{
	OnDestroy();
}

void CEggLockGUI::OnRender( IDXGISwapChain* pSwapChain )
{
	...
	else
	{
		if ( !m_pRenderTargetView )
		{
			...
			{
				DEV_LOG( "[error] CEggLockGUI::OnRender: #1\n" );
				return;
			}
...
auto CEggLockGUI::OnReopenGUI() -> void
{
...
LRESULT WINAPI CEggLockGUI::GUI_WndProc( HWND hwnd , UINT uMsg , WPARAM wParam , LPARAM lParam )
{
	...
	if ( GetEggLockGUI()->m_bInit )
	{
		if ( uMsg == WM_KEYUP && wParam == VK_INSERT )
			GetEggLockGUI()->OnReopenGUI();

		if ( GetEggLockGUI()->IsVisible() && ImGui_ImplWin32_WndProcHandler( hwnd , uMsg , wParam , lParam ) == 0 )
			return true;
	}

	return CallWindowProcA( GetEggLockGUI()->m_WndProc_o , hwnd , uMsg , wParam , lParam );
}

auto CEggLockGUI::FreeTypeBuild::PreNewFrame() -> bool
{
	if ( !WantRebuild )
		return false;
...
auto CEggLockGUI::ClearRenderTargetView() -> void
{
	if ( m_pRenderTargetView )
	{
...
auto GetEggLockGUI() -> CEggLockGUI*
{
	return &g_CEggLockGUI;
}
